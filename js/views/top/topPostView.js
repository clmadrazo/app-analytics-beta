define([
        ...
        , 'text!templatesFolder/top/top_post.html'
        ,'models/ca/caModel'
        , 'bootstrap-select'
        , 'moment',
        , 'locale'
    ...
    , function( doT, BaseView, HeaderNavTemplate,TopPostTemplate, CaModel) {

        var CaView = BaseView.extend({
            el : '#main-wrapper',
            headerNav : doT.template(HeaderNavTemplate),
            template : doT.template(TopPostTemplate),
            idleInterval: 0,
            caModel : new CaModel(),
            events: {
            ...
            _searchPosts: function(event){
                ...
                if( searchString != "")
                {
                    ...
                    $(".top_post_1 .table-post tbody>tr").hide();
                    $(".top_post_1 .table-post td:contains('" + searchString + "')").parent("tr").show();
                }
                else
                {
                    // When there is no input or clean again, show everything back
                    $(".top_post_1 .table-post tbody>tr").show();
                }
                ...
                this.caModel.fetch({
                    ...
                    complete: function (response) {
                        if (response.status == 400) {
                            ...
                            $('.top_post_1 .table-post tbody').empty();
                            var result = response.responseJSON.result;
                            for (var i = 0; i < result.length; i++) {
                                $('.top_post_1 .table-post tbody').append(
                                    '<tr>'+
                            ...
                            $(that.overlay).hide();
                            $('.top_post_1 .table-post tbody').append('<tr><td>'+Polyglo.t('general.norecords')+'</td></tr>');
                        }
                    }});
            },
            render : function(){
                if (localstorage.get('logged') === false)
                    ...
                    $('.div_back').html('<a href="#analytics"><i class="fa fa-arrow-left"></i>Top posts</a>');
                    this.buildView();
                }
            },
            close: function(){
