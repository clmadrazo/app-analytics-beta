define([
        ...
        , 'text!templatesFolder/top/top_author.html'
        ,'models/ca/caModel'
        , 'bootstrap-select'
        , 'moment',
        , 'locale'
    ...
    , function( doT, BaseView,HeaderNavTemplate, TopAuthorTemplate, CaModel) {

        var CaView = BaseView.extend({
            el : '#main-wrapper',
            headerNav : doT.template(HeaderNavTemplate),
            template : doT.template(TopAuthorTemplate),
            idleInterval: 0,
            caModel : new CaModel(),
            events: {
            ...
            _searchPosts: function(event){
                ...
                if( searchString != "")
                {
                    ...
                    $(".top_author .table-post tbody>tr").hide();
                    $(".top_author .table-post td:contains('" + searchString + "')").parent("tr").show();
                }
                else
                {
                    // When there is no input or clean again, show everything back
                    $(".top_author .table-post tbody>tr").show();
                }
                ...
                this.caModel.fetch({
                    ...
                    complete: function (response) {
                        if (response.status == 400) {
                            ...
                            $('.top_author_select .top_author_1 tbody').empty();
                            var result = response.responseJSON.result,
                                length = 3;
                            if (result.length < 3)
                                length = result.length;
                            for (var i = 0; i < length; i++) {
                                $('.top_author_select .top_author_1 tbody').append(
                                    '<tr>' +
                            ...
                            length = 10;
                            $('.top_author_select .top_author_2 tbody').empty();
                            if (result.length < 10)
                                length = result.length;
                            for (var i = 0; i < length; i++) {
                                $('.top_author_select .top_author_2 tbody').html(
                                    '<tr>'+
                                        '<td>' + result[i].author + '</td>'+
                                        '<td>' + result[i].title + '</td>'+
                                        '<td>' + result[i].shares + '</td>'+
                            ...
                            $(that.overlay).hide();
                            $('.top_author_select .top_author_2 tbody').append('<tr><td>'+Polyglo.t('general.norecords')+'</td></tr>');
                            $('.top_author_select .top_author_1 tbody').append('<tr><td>'+Polyglo.t('general.norecords')+'</td></tr>');
                        }
                    }});
            },
            render : function(){
                if (localstorage.get('logged') === false)
                    ...
                    $('.div_back').html('<a href="#analytics"><i class="fa fa-arrow-left"></i>Top autores</a>');
                    this.buildView();
                }
            },
            close: function(){
