...
class ProfileHelper implements ServiceLocatorAwareInterface
{
    ...
    public function getUserBy($requestData)
    {
        ...
        } elseif(isset($requestData[0]['profileUrl'])) {
            $user = $this->getUserByProfileUrl($requestData[0]['profileUrl']);
        }
    ...
    public function getUserByEmail($email)
    {
        ...
        $user = $em->getRepository('User\Entity\User')
            ->findOneBy(array('email' => $email, 'status' => User::STATUS_ACTIVE));
    ...
    public function generateToken($user)
    {
        ...
        $t['token'] = bin2hex(openssl_random_pseudo_bytes(16));
        $t['refresh'] = bin2hex(openssl_random_pseudo_bytes(16));

        $token->setUser($user);
        $token->setValue($t['token']);
        $token->setRefresh($t['refresh']);
        $token->setCreated();
