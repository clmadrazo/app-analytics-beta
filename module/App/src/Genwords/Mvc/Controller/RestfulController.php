...
/**
 * App simplified RESTful controller. What are the benefits
...
class RestfulController extends AbstractRestfulController
{
    ...
    public function onDispatch(MvcEvent $e)
    {
        ...
        if (!$this->requestHasContentType($request, self::CONTENT_TYPE_JSON)) {
            ...
            if ($routeName !== 'registration' && $routeName !== 'set-password' && $routeName !== 'activateUser' && $routeName !== 'forgot-password' && $routeName !== 'valid-reset-password' && $routeName !== 'login' && $routeName !== 'checkAuth' && $routeName !== 'refreshToken' && $routeName !== 'validateToken') {
                $response->setStatusCode(400);
                return $response->setContent(Json::encode((object)['error' => self::PROCESS_REQUEST_ERROR]));
            }
            ...
            if (!$this->_checkRequestContent($request)
                || !$this->_userIsAllowedToExecute($token, $routeName)) {
                $response->setStatusCode(400);
                return $response->setContent(Json::encode((object)['error' => self::PROCESS_REQUEST_ERROR]));
            } else {
    ...
    protected function _checkRequestContent(\Zend\Http\PhpEnvironment\Request $request)
    {
        ...
        if (empty($content)) {
            ...
            $oParams = is_array($oParams)
                ? $oParams
                : (is_object($oParams) ? [$oParams] : false);
    ...
    public function getRequestId()
    {
        if (!isset($this->requestId)) {
            $this->requestId = md5(\App\Util\DateUtils::rightNow() . microtime());
        }
